...
def get_token():
    ...
    return info
class AirSpiderZbjggs(feapder.AirSpider):
    '''
                       函数入口
                        :return:
               '''
    def start_requests(self):
            ...
            for page in range(101,1006):
                if page==1:
                    url="http://zfcg.szggzy.com:8081/gsgg/002001/002001004/list.html"
                    yield feapder.Request(url, callback=self.parse1, verify=False, method="GET")
                elif 101>page>1:
                    url=f"http://zfcg.szggzy.com:8081/gsgg/002001/002001004/{page}.html"
                    yield feapder.Request(url, callback=self.parse1, verify=False, method="GET")
                else:
                    url=f"http://zfcg.szggzy.com:8081/gsgg/002001/002001004/list.html?categoryNum=002001004&pageIndex={page}"
    ...
    def parse2(self, request, response):
        ...
        data = {
            "params": "{\"siteGuid\":\"\",\"categoryNum\":\"002001004\",\"pageIndex\":\"%s\",\"pageSize\":1,\"controlname\":\"subpagelist\",\"ImgGuid\":\"%s\",\"YZM\":\"%s\"}" % (
...
if __name__ == "__main__":
    AirSpiderZbjggs(thread_count=1).start()
