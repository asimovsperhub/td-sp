...
def get_token():
    ...
    return info
class AirSpiderHtgg(feapder.AirSpider):
    '''
                       函数入口
                        :return:
               '''
    def start_requests(self):
            ...
            for page in range(1,1176):
                if page==1:
                    url="http://zfcg.szggzy.com:8081/gsgg/002002/list.html"
                    yield feapder.Request(url, callback=self.parse1, verify=False, method="GET")
                elif 101>page>1:
                    url=f"http://zfcg.szggzy.com:8081/gsgg/002002/{page}.html"
                    yield feapder.Request(url, callback=self.parse1, verify=False, method="GET")
                else:
                    url=f"http://zfcg.szggzy.com:8081/gsgg/002002/list.html?categoryNum=002002&pageIndex={page}"
    ...
    def parse2(self, request, response):
        ...
        data = {
            "params": "{\"siteGuid\":\"\",\"categoryNum\":\"002002\",\"pageIndex\":\"%s\",\"pageSize\":1,\"controlname\":\"subpagelist\",\"ImgGuid\":\"%s\",\"YZM\":\"%s\"}" % (
...
if __name__ == "__main__":
    AirSpiderHtgg(thread_count=1).start()
