...
class flk(feapder.AirSpider):
    ...
    def details_pm(self, request, response):
        ...
        it.pdf = item.get("pdf")
        it.url = item.get("url")
        it.status = item.get("status")
        it.level = item.get("level")
        yield it

    def download_file(self, request, response):
        dir = f"/mnt/fvdownload/"
